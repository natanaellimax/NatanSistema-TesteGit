// ===== SISTEMA NatanSistem HB - VERSÃO 1.50 =====
...
function debug(msg, data) {
  console.log('[SistemNatan] ' + msg, data || '');
}
...
function criarPopupEstilizado(titulo, mensagem, callback) {
  ...
  popup.innerHTML = `
    <div style="margin-bottom: 20px;">
      <img src="logo.png" alt="Logo" style="width: 60px; height: 60px; margin-bottom: 15px;">
    </div>
...
function carregarDados() {
  try {
    var dadosUsuarios = localStorage.getItem('usuarios');
    if (dadosUsuarios) {
      usuarios = JSON.parse(dadosUsuarios);
      debug('Usuários carregados', usuarios);
    }
    
    var usuarioAtualStorage = localStorage.getItem('usuario_atual');
    if (usuarioAtualStorage) {
      ...
      var carteiraData = localStorage.getItem('carteira_' + usuarioAtual);
      if (carteiraData) carteira = JSON.parse(carteiraData);
      
      var extratoData = localStorage.getItem('extrato_' + usuarioAtual);
      if (extratoData) extrato = JSON.parse(extratoData);
      
      var ordensData = localStorage.getItem('ordens_' + usuarioAtual);
...
function salvarDados() {
  try {
    ...
    if (usuarioAtual) {
      localStorage.setItem('usuario_atual', usuarioAtual);
...
function realizarLogin() {
  ...
  // Chama a função que exibe o popup com saudação dinâmica
  saudarEExibirPopup(cpf);
}

// Função que exibe popup com saudação personalizada conforme horário
function saudarEExibirPopup(cpf) {
  const nome = usuarios[cpf].nome || 'Usuário';
  const hora = new Date().getHours();
  let saudacao;

  if (hora >= 5 && hora < 12) {
    saudacao = 'Tenha uma ótima manhã cheia de oportunidades!';
  } else if (hora >= 12 && hora < 18) {
    saudacao = 'Tenha uma ótima tarde cheia de oportunidades!';
  } else {
    saudacao = 'Tenha uma ótima noite cheia de oportunidades!';
  }

  const mensagem = `Olá, ${nome}!<br><br>${saudacao}<br><br>Você fez login com sucesso ao Sistema NatanSistem HB.`;
...
function verificarLocalStorage() {
  ...
  console.log('usuario_atual:', localStorage.getItem('usuario_atual'));
